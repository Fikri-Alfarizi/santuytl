...
@section('title', 'Remove Role Discord')
@section('content')
<div class="container">
    <h2>Remove Role dari User Discord</h2>
    <form method="POST" action="{{ route('admin.discord.remove-role') }}">
        @csrf
            ...
            <select name="user_id" class="form-control select2-user" required>
                ...
                @foreach($users as $user)
                    <option value="{{ $user['id'] }}" {{ old('user_id') == $user['id'] ? 'selected' : '' }}>
                        @if(!empty($user['guild']))[{{ $user['guild'] }}] @endif{{ $user['username'] ?? 'User' }}@if(!empty($user['discriminator']))#{{ $user['discriminator'] }}@endif ({{ $user['id'] }})
                    </option>
                @endforeach
            </select>
        </div>
